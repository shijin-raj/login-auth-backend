...
app.post("/refresh", (req, res) => {
  ...
  if (hasAccessToken) {
    ...
    try {
      const decoded = jwt.verify(accessToken, process.env.ACCESS_SECRET_KEY);
      req.user = decoded;
      res.json({ message: "Login successful", user: req.user, status: true });
    } catch (err) {
  ...
  if (checkRefreshToken) {
    if (hasRefreshToken) {
      ...
      try {
        ...
        res.json({ message: "Token refreshed successfully", status: true });
      } catch (err) {
        res
          .status(200)
      ...
      return res
        .status(200)
        .json({ message: "No refresh token provided", status: false });
    }
  }
});
